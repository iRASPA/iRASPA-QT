...
OpenGLCrystalCylinderObjectSelectionWorleyNoise3DShader::OpenGLCrystalCylinderObjectSelectionWorleyNoise3DShader(OpenGLCrystalCylinderSelectionInstanceShader &instanceShader, OpenGLCrystalCylinderObjectShader &crystalCylinderShader):
     _instanceShader(instanceShader), _crystalCylinderShader(crystalCylinderShader)
{
}
...
void OpenGLCrystalCylinderObjectSelectionWorleyNoise3DShader::paintGL(GLuint structureUniformBuffer)
{
  ...
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
    {
      ...
      {
        if (RKRenderCrystalPrimitiveCylinderObjectsSource* source = dynamic_cast<RKRenderCrystalPrimitiveCylinderObjectsSource*>(_renderStructures[i][j].get()))
        {
          if(object->primitiveSelectionStyle() == RKSelectionStyle::WorleyNoise3D)
          {
            if(_renderStructures[i][j]->isVisible() && _crystalCylinderShader._numberOfIndices[i][j]>0 && _instanceShader._numberOfDrawnAtoms[i][j]>0)
            {
              ...
              glDrawElementsInstanced(GL_TRIANGLES, static_cast<GLsizei>(_crystalCylinderShader._numberOfIndices[i][j]), GL_UNSIGNED_SHORT, nullptr, static_cast<GLsizei>(_instanceShader._numberOfDrawnAtoms[i][j]));
              check_gl_error();
              glBindVertexArray(0);
            }
          }
...
void OpenGLCrystalCylinderObjectSelectionWorleyNoise3DShader::deleteBuffers()
{
  for(size_t i=0;i<_renderStructures.size();i++)
  {
...
void OpenGLCrystalCylinderObjectSelectionWorleyNoise3DShader::generateBuffers()
{
...
void OpenGLCrystalCylinderObjectSelectionWorleyNoise3DShader::reloadData()
{
  initializeVertexArrayObject();
}

void OpenGLCrystalCylinderObjectSelectionWorleyNoise3DShader::initializeVertexArrayObject()
{
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
...
void OpenGLCrystalCylinderObjectSelectionWorleyNoise3DShader::loadShader(void)
{
  GLuint vertexShader;
  GLuint fragmentShader;

  vertexShader=compileShaderOfType(GL_VERTEX_SHADER, OpenGLCrystalCylinderObjectSelectionWorleyNoise3DShader::_vertexShaderSource.c_str());
  fragmentShader=compileShaderOfType(GL_FRAGMENT_SHADER, OpenGLCrystalCylinderObjectSelectionWorleyNoise3DShader::_fragmentShaderSource.c_str());
...
const std::string OpenGLCrystalCylinderObjectSelectionWorleyNoise3DShader::_vertexShaderSource  =
...
const std::string OpenGLCrystalCylinderObjectSelectionWorleyNoise3DShader::_fragmentShaderSource =
OpenGLUniformStringLiterals::OpenGLVersionStringLiteral +
OpenGLUniformStringLiterals::OpenGLFrameUniformBlockStringLiteral +
OpenGLUniformStringLiterals::OpenGLStructureUniformBlockStringLiteral +
OpenGLUniformStringLiterals::OpenGLLightUniformBlockStringLiteral +
