...
OpenGLCrystalPolygonalPrismObjectSelectionGlowShader::OpenGLCrystalPolygonalPrismObjectSelectionGlowShader(OpenGLCrystalPolygonalPrismSelectionInstanceShader &instanceShader, OpenGLCrystalPolygonalPrismObjectShader &crystalPolygonalPrismShader):
     _instanceShader(instanceShader), _crystalPolygonalPrismShader(crystalPolygonalPrismShader)
{
}


void OpenGLCrystalPolygonalPrismObjectSelectionGlowShader::setRenderStructures(std::vector<std::vector<std::shared_ptr<RKRenderStructure>>> structures)
{
  deleteBuffers();
...
void OpenGLCrystalPolygonalPrismObjectSelectionGlowShader::paintGL(GLuint structureUniformBuffer)
{
  ...
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
    {
      if (RKRenderPrimitiveObjectsSource* source = dynamic_cast<RKRenderPrimitiveObjectsSource*>(_renderStructures[i][j].get()))
      {
        ...
        {
          if(source->primitiveSelectionStyle() == RKSelectionStyle::glow)
          {
            if(_renderStructures[i][j]->isVisible() && _crystalPolygonalPrismShader._numberOfIndices[i][j]>0 && _instanceShader._numberOfDrawnAtoms[i][j]>0)
            {
              ...
              // draw only the front-faces for the glow style
              glDrawElementsInstanced(GL_TRIANGLES, static_cast<GLsizei>(_crystalPolygonalPrismShader._numberOfIndices[i][j]), GL_UNSIGNED_SHORT, nullptr, static_cast<GLsizei>(_instanceShader._numberOfDrawnAtoms[i][j]));
              check_gl_error();
              glBindVertexArray(0);
            }
          }
...
void OpenGLCrystalPolygonalPrismObjectSelectionGlowShader::deleteBuffers()
{
  for(size_t i=0;i<_renderStructures.size();i++)
  {
...
void OpenGLCrystalPolygonalPrismObjectSelectionGlowShader::generateBuffers()
{
...
void OpenGLCrystalPolygonalPrismObjectSelectionGlowShader::reloadData()
{
  initializeVertexArrayObject();
}

void OpenGLCrystalPolygonalPrismObjectSelectionGlowShader::initializeVertexArrayObject()
{
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
    {
      if (RKRenderCrystalPrimitivePolygonalPrimsObjectsSource* object = dynamic_cast<RKRenderCrystalPrimitivePolygonalPrimsObjectsSource*>(_renderStructures[i][j].get()))
      {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, _crystalPolygonalPrismShader._vertexBuffer[i][j]);
        check_gl_error();
        ...
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _crystalPolygonalPrismShader._indexBuffer[i][j]);
...
void OpenGLCrystalPolygonalPrismObjectSelectionGlowShader::loadShader(void)
{
  GLuint vertexShader;
  GLuint fragmentShader;

  vertexShader=compileShaderOfType(GL_VERTEX_SHADER, OpenGLCrystalPolygonalPrismObjectSelectionGlowShader::_vertexShaderSource.c_str());
  fragmentShader=compileShaderOfType(GL_FRAGMENT_SHADER, OpenGLCrystalPolygonalPrismObjectSelectionGlowShader::_fragmentShaderSource.c_str());
...
const std::string OpenGLCrystalPolygonalPrismObjectSelectionGlowShader::_vertexShaderSource  =
...
const std::string OpenGLCrystalPolygonalPrismObjectSelectionGlowShader::_fragmentShaderSource =
OpenGLVersionStringLiteral +
OpenGLFrameUniformBlockStringLiteral +
OpenGLStructureUniformBlockStringLiteral +
OpenGLLightUniformBlockStringLiteral +
