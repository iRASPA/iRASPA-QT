...
void OpenGLCrystalPolygonalPrismObjectSelectionWorleyNoise3DShader::paintGL(GLuint structureUniformBuffer)
{
  ...
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
    {
      ...
      {
        if (RKRenderCrystalPrimitivePolygonalPrimsObjectsSource* source = dynamic_cast<RKRenderCrystalPrimitivePolygonalPrimsObjectsSource*>(_renderStructures[i][j].get()))
        {
...
void OpenGLCrystalPolygonalPrismObjectSelectionWorleyNoise3DShader::deleteBuffers()
{
  for(size_t i=0;i<_renderStructures.size();i++)
  {
...
void OpenGLCrystalPolygonalPrismObjectSelectionWorleyNoise3DShader::generateBuffers()
{
...
void OpenGLCrystalPolygonalPrismObjectSelectionWorleyNoise3DShader::reloadData()
{
  initializeVertexArrayObject();
}

void OpenGLCrystalPolygonalPrismObjectSelectionWorleyNoise3DShader::initializeVertexArrayObject()
{
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
...
void OpenGLCrystalPolygonalPrismObjectSelectionWorleyNoise3DShader::loadShader(void)
{
  GLuint vertexShader;
  GLuint fragmentShader;

  vertexShader=compileShaderOfType(GL_VERTEX_SHADER, OpenGLCrystalPolygonalPrismObjectSelectionWorleyNoise3DShader::_vertexShaderSource.c_str());
  fragmentShader=compileShaderOfType(GL_FRAGMENT_SHADER, OpenGLCrystalPolygonalPrismObjectSelectionWorleyNoise3DShader::_fragmentShaderSource.c_str());
