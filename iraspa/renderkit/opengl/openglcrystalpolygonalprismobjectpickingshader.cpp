  ...
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
    {
      if (RKRenderPrimitiveObjectsSource* source = dynamic_cast<RKRenderPrimitiveObjectsSource*>(_renderStructures[i][j].get()))
      {
        ...
        {
          if(source->drawAtoms() && _renderStructures[i][j]->isVisible() && _crystalPolygonalPrismShader._numberOfIndices[i][j]>0 && _crystalPolygonalPrismShader._numberOfDrawnAtoms[i][j]>0)
          {
