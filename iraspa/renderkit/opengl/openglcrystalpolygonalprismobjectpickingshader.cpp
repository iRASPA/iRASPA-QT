...
void OpenGLCrystalPolygonalPrismObjectPickingShader::setRenderStructures(std::vector<std::vector<std::shared_ptr<RKRenderStructure>>> structures)
{
  deleteBuffers();
...
void OpenGLCrystalPolygonalPrismObjectPickingShader::deleteBuffers()
{
...
void OpenGLCrystalPolygonalPrismObjectPickingShader::generateBuffers()
{
...
void OpenGLCrystalPolygonalPrismObjectPickingShader::paintGL(GLuint structureUniformBuffer)
{
  ...
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
    {
      ...
      {
        if(_renderStructures[i][j]->drawAtoms() && _renderStructures[i][j]->isVisible() && _crystalPolygonalPrismShader._numberOfIndices[i][j]>0 && _crystalPolygonalPrismShader._numberOfDrawnAtoms[i][j]>0)
        {
          ...
          glDrawElementsInstanced(GL_TRIANGLES, static_cast<GLsizei>(_crystalPolygonalPrismShader._numberOfIndices[i][j]), GL_UNSIGNED_SHORT, nullptr, static_cast<GLsizei>(_crystalPolygonalPrismShader._numberOfDrawnAtoms[i][j]));
          check_gl_error();
          glBindVertexArray(0);
        }
      }
...
void OpenGLCrystalPolygonalPrismObjectPickingShader::reloadData()
{
  initializeVertexArrayObject();
}

void OpenGLCrystalPolygonalPrismObjectPickingShader::initializeVertexArrayObject()
{
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
    {
      if (RKRenderCrystalPrimitivePolygonalPrimsObjectsSource* object = dynamic_cast<RKRenderCrystalPrimitivePolygonalPrimsObjectsSource*>(_renderStructures[i][j].get()))
      {
        ...
        glBindBuffer(GL_ARRAY_BUFFER,_crystalPolygonalPrismShader._instancePositionBuffer[i][j]);
...
void OpenGLCrystalPolygonalPrismObjectPickingShader::loadShader(void)
{
  GLuint vertexShader;
  GLuint fragmentShader;
  vertexShader=compileShaderOfType(GL_VERTEX_SHADER,OpenGLCrystalPolygonalPrismObjectPickingShader::_vertexShaderSource.c_str());
  fragmentShader=compileShaderOfType(GL_FRAGMENT_SHADER,OpenGLCrystalPolygonalPrismObjectPickingShader::_fragmentShaderSource.c_str());
