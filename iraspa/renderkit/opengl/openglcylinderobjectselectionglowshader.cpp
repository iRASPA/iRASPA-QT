...
OpenGLCylinderObjectSelectionGlowShader::OpenGLCylinderObjectSelectionGlowShader(OpenGLCylinderSelectionInstanceShader &instanceShader, OpenGLCylinderObjectShader &cylinderShader):
     _instanceShader(instanceShader), _cylinderShader(cylinderShader)
{
}


void OpenGLCylinderObjectSelectionGlowShader::setRenderStructures(std::vector<std::vector<std::shared_ptr<RKRenderStructure>>> structures)
{
  deleteBuffers();
...
void OpenGLCylinderObjectSelectionGlowShader::paintGL(GLuint structureUniformBuffer)
{
  ...
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
    {
      ...
      {
        if (RKRenderPrimitiveCylinderObjectsSource* source = dynamic_cast<RKRenderPrimitiveCylinderObjectsSource*>(_renderStructures[i][j].get()))
        {
...
void OpenGLCylinderObjectSelectionGlowShader::deleteBuffers()
{
  for(size_t i=0;i<_renderStructures.size();i++)
  {
...
void OpenGLCylinderObjectSelectionGlowShader::generateBuffers()
{
...
void OpenGLCylinderObjectSelectionGlowShader::reloadData()
{
  initializeVertexArrayObject();
}

void OpenGLCylinderObjectSelectionGlowShader::initializeVertexArrayObject()
{
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
...
void OpenGLCylinderObjectSelectionGlowShader::loadShader(void)
{
  GLuint vertexShader;
  GLuint fragmentShader;

  vertexShader=compileShaderOfType(GL_VERTEX_SHADER, OpenGLCylinderObjectSelectionGlowShader::_vertexShaderSource.c_str());
  fragmentShader=compileShaderOfType(GL_FRAGMENT_SHADER, OpenGLCylinderObjectSelectionGlowShader::_fragmentShaderSource.c_str());
...
const std::string OpenGLCylinderObjectSelectionGlowShader::_vertexShaderSource  =
...
const std::string OpenGLCylinderObjectSelectionGlowShader::_fragmentShaderSource =
OpenGLVersionStringLiteral +
OpenGLFrameUniformBlockStringLiteral +
OpenGLStructureUniformBlockStringLiteral +
OpenGLLightUniformBlockStringLiteral +
