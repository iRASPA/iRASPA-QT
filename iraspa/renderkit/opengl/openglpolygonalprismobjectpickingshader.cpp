...
OpenGLPolygonalPrismObjectPickingShader::OpenGLPolygonalPrismObjectPickingShader(OpenGLObjectShader &objectShader):
      _polygonalPrismShader(objectShader._polygonalPrismObjectShader)
{
}


void OpenGLPolygonalPrismObjectPickingShader::setRenderStructures(std::vector<std::vector<std::shared_ptr<RKRenderStructure>>> structures)
{
  deleteBuffers();
...
void OpenGLPolygonalPrismObjectPickingShader::deleteBuffers()
{
...
void OpenGLPolygonalPrismObjectPickingShader::generateBuffers()
{
...
void OpenGLPolygonalPrismObjectPickingShader::paintGL(GLuint structureUniformBuffer)
{
  ...
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
    {
      ...
      {
        if(_renderStructures[i][j]->drawAtoms() && _renderStructures[i][j]->isVisible() && _polygonalPrismShader._numberOfIndices[i][j]>0 && _polygonalPrismShader._numberOfDrawnAtoms[i][j]>0)
        {
          ...
          glDrawElementsInstanced(GL_TRIANGLES, static_cast<GLsizei>(_polygonalPrismShader._numberOfIndices[i][j]), GL_UNSIGNED_SHORT, nullptr, static_cast<GLsizei>(_polygonalPrismShader._numberOfDrawnAtoms[i][j]));
          check_gl_error();
          glBindVertexArray(0);
        }
      }
...
void OpenGLPolygonalPrismObjectPickingShader::reloadData()
{
  initializeVertexArrayObject();
}

void OpenGLPolygonalPrismObjectPickingShader::initializeVertexArrayObject()
{
  ...
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
    {
      if (RKRenderPrimitivePolygonalPrimsObjectsSource* object = dynamic_cast<RKRenderPrimitivePolygonalPrimsObjectsSource*>(_renderStructures[i][j].get()))
      {
        ...
        glBindBuffer(GL_ARRAY_BUFFER,_polygonalPrismShader._instancePositionBuffer[i][j]);
...
void OpenGLPolygonalPrismObjectPickingShader::loadShader(void)
{
  GLuint vertexShader;
  GLuint fragmentShader;
  vertexShader=compileShaderOfType(GL_VERTEX_SHADER,OpenGLPolygonalPrismObjectPickingShader::_vertexShaderSource.c_str());
  fragmentShader=compileShaderOfType(GL_FRAGMENT_SHADER,OpenGLPolygonalPrismObjectPickingShader::_fragmentShaderSource.c_str());
...
const std::string  OpenGLPolygonalPrismObjectPickingShader::_vertexShaderSource =
...
const std::string  OpenGLPolygonalPrismObjectPickingShader::_fragmentShaderSource =
OpenGLVersionStringLiteral +
OpenGLFrameUniformBlockStringLiteral +
OpenGLStructureUniformBlockStringLiteral +
R"foo(
