...
void OpenGLCylinderObjectPickingShader::deleteBuffers()
{
...
void OpenGLCylinderObjectPickingShader::generateBuffers()
{
...
void OpenGLCylinderObjectPickingShader::paintGL(GLuint structureUniformBuffer)
{
...
void OpenGLCylinderObjectPickingShader::reloadData()
{
  initializeVertexArrayObject();
}

void OpenGLCylinderObjectPickingShader::initializeVertexArrayObject()
{
  ...
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
    {
      if (RKRenderPrimitiveCylinderObjectsSource* object = dynamic_cast<RKRenderPrimitiveCylinderObjectsSource*>(_renderStructures[i][j].get()))
      {
        ...
        glBindBuffer(GL_ARRAY_BUFFER,_cylinderShader._instancePositionBuffer[i][j]);
...
void OpenGLCylinderObjectPickingShader::loadShader(void)
{
  GLuint vertexShader;
  GLuint fragmentShader;
  vertexShader=compileShaderOfType(GL_VERTEX_SHADER,OpenGLCylinderObjectPickingShader::_vertexShaderSource.c_str());
  fragmentShader=compileShaderOfType(GL_FRAGMENT_SHADER,OpenGLCylinderObjectPickingShader::_fragmentShaderSource.c_str());
