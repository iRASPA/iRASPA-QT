...
void main()
{
  ...
  outN = (frameUniforms.normalMatrix * structureUniforms.modelMatrix * orientationMatrix * vertexNormal).xyz;
  vec4 worldPos = vec4((orientationMatrix * (scale * pos) + pos1).xyz, 1.0);
  vec4 P = frameUniforms.viewMatrix * structureUniforms.modelMatrix * worldPos;
  outL = (lightUniforms.lights[0].position - P * lightUniforms.lights[0].position.w).xyz;
  outV = -P.xyz;
  gl_Position = frameUniforms.mvpMatrix * structureUniforms.modelMatrix * worldPos;
}
