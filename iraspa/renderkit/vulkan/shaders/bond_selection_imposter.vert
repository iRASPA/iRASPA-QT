...
layout(location = 0) flat out vec4 outColor1;
layout(location = 1) flat out vec4 outColor2;
layout(location = 2) flat out vec2 outMixParam;
layout(location = 3) out vec3 outFragPos;
layout(location = 4) flat out vec3 outPointA;
layout(location = 5) flat out vec3 outPointB;
layout(location = 6) flat out float outRadius;
layout(location = 7) flat out vec3 outAxisX;
layout(location = 8) flat out vec3 outAxisZ;
...
void main()
{
  ...
  outMixParam.x = clamp(structureUniforms.atomScaleFactor, 0.0, 0.7) * instanceScale.x;
  outMixParam.y = 1.0 - clamp(structureUniforms.atomScaleFactor, 0.0, 0.7) * instanceScale.z;
  outColor1 = instanceColor1;
  outColor2 = instanceColor2;
  ...
  outAxisX = normalize((mv * vec4(-v1, 0.0)).xyz);
  outAxisZ = normalize((mv * vec4(-v2, 0.0)).xyz);
