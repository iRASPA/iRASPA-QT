...
layout(location = 6) in vec2 inAoUV;
layout(location = 7) in vec3 inBaseColor;
...
void main()
{
  ivec2 aoTexSize = max(textureSize(ambientOcclusionTexture, 0), ivec2(1));
  vec2 inverseTextureSize = 1.0 / vec2(aoTexSize);
  vec2 aoUV = ribbonAmbientOcclusionUV(inAoUV);
  float aoSample = 1.0;
  if (structureUniforms.ribbonAmbientOcclusion != 0 && ribbonAODebug.fastInteractionShading == 0)
  {
  ...
  if (ribbonAODebug.mode != 0)
  {
    ...
    {
      vec2 texelCoord = aoUV * vec2(aoTexSize);
    ...
    {
      vFragColor = vec4(inBaseColor, 1.0);
      return;
    }
    ...
    if (ribbonAODebug.mode == 6 || ribbonAODebug.mode == 7)
    {
  ...
  vec3 ambient = inAmbient;
  vec3 diffuse = max(dot(N, L), 0.0) * inDiffuse;
  vec3 specular = pow(max(dot(R, V), 0.0), lightUniforms.lights[0].shininess + structureUniforms.ribbonShininess) * inSpecular;
