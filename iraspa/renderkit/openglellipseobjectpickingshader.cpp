...
void OpenGLEllipseObjectPickingShader::deleteBuffers()
{
...
void OpenGLEllipseObjectPickingShader::generateBuffers()
{
...
void OpenGLEllipseObjectPickingShader::paintGL(GLuint structureUniformBuffer)
{
  ...
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
    {
      ...
      {
        if(_renderStructures[i][j]->drawAtoms() && _renderStructures[i][j]->isVisible() && _ellipseShader._numberOfIndices[i][j]>0 && _ellipseShader._numberOfDrawnAtoms[i][j]>0)
        {
          ...
          glDrawElementsInstanced(GL_TRIANGLES, static_cast<GLsizei>(_ellipseShader._numberOfIndices[i][j]), GL_UNSIGNED_SHORT, nullptr, static_cast<GLsizei>(_ellipseShader._numberOfDrawnAtoms[i][j]));
          check_gl_error();
          glBindVertexArray(0);
        }
      }
...
void OpenGLEllipseObjectPickingShader::reloadData()
{
  initializeVertexArrayObject();
}

void OpenGLEllipseObjectPickingShader::initializeVertexArrayObject()
{
  ...
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
    {
      if (RKRenderPrimitiveEllipsoidObjectsSource* object = dynamic_cast<RKRenderPrimitiveEllipsoidObjectsSource*>(_renderStructures[i][j].get()))
      {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, _ellipseShader._vertexBuffer[i][j]);
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _ellipseShader._indexBuffer[i][j]);
        ...
        glBindBuffer(GL_ARRAY_BUFFER,_ellipseShader._instancePositionBuffer[i][j]);
...
void OpenGLEllipseObjectPickingShader::loadShader(void)
{
  GLuint vertexShader;
  GLuint fragmentShader;
  vertexShader=compileShaderOfType(GL_VERTEX_SHADER,OpenGLEllipseObjectPickingShader::_vertexShaderSource.c_str());
  fragmentShader=compileShaderOfType(GL_FRAGMENT_SHADER,OpenGLEllipseObjectPickingShader::_fragmentShaderSource.c_str());
