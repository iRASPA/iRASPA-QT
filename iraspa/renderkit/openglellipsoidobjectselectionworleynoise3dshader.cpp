...
OpenGLEllipsoidObjectSelectionWorleyNoise3DShader::OpenGLEllipsoidObjectSelectionWorleyNoise3DShader(OpenGLEllipsoidSelectionInstanceShader &instanceShader, OpenGLEllipseObjectShader &ellipsoidShader):
...
void OpenGLEllipsoidObjectSelectionWorleyNoise3DShader::paintGL(GLuint structureUniformBuffer)
{
  ...
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
    {
      if (RKRenderPrimitiveEllipsoidObjectsSource* object = dynamic_cast<RKRenderPrimitiveEllipsoidObjectsSource*>(_renderStructures[i][j].get()))
      {
        ...
        {
          if(_renderStructures[i][j]->isVisible() && _ellipsoidShader._numberOfIndices[i][j]>0 && _instanceShader._numberOfDrawnAtoms[i][j]>0)
          {
            ...
            glDrawElementsInstanced(GL_TRIANGLE_STRIP, static_cast<GLsizei>(_ellipsoidShader._numberOfIndices[i][j]), GL_UNSIGNED_SHORT, nullptr, static_cast<GLsizei>(_instanceShader._numberOfDrawnAtoms[i][j]));
            check_gl_error();
            glBindVertexArray(0);
          }
        }
...
void OpenGLEllipsoidObjectSelectionWorleyNoise3DShader::deleteBuffers()
{
  for(size_t i=0;i<_renderStructures.size();i++)
  {
...
void OpenGLEllipsoidObjectSelectionWorleyNoise3DShader::generateBuffers()
{
...
void OpenGLEllipsoidObjectSelectionWorleyNoise3DShader::reloadData()
{
  initializeVertexArrayObject();
}

void OpenGLEllipsoidObjectSelectionWorleyNoise3DShader::initializeVertexArrayObject()
{
  for(size_t i=0;i<_renderStructures.size();i++)
  {
    for(size_t j=0;j<_renderStructures[i].size();j++)
    {
      if (RKRenderPrimitiveEllipsoidObjectsSource* object = dynamic_cast<RKRenderPrimitiveEllipsoidObjectsSource*>(_renderStructures[i][j].get()))
      {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, _ellipsoidShader._vertexBuffer[i][j]);
        check_gl_error();
        ...
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _ellipsoidShader._indexBuffer[i][j]);
...
void OpenGLEllipsoidObjectSelectionWorleyNoise3DShader::loadShader(void)
{
  GLuint vertexShader;
  GLuint fragmentShader;

  vertexShader=compileShaderOfType(GL_VERTEX_SHADER, OpenGLEllipsoidObjectSelectionWorleyNoise3DShader::_vertexShaderSource.c_str());
  fragmentShader=compileShaderOfType(GL_FRAGMENT_SHADER, OpenGLEllipsoidObjectSelectionWorleyNoise3DShader::_fragmentShaderSource.c_str());
...
const std::string OpenGLEllipsoidObjectSelectionWorleyNoise3DShader::_vertexShaderSource  =
...
const std::string OpenGLEllipsoidObjectSelectionWorleyNoise3DShader::_fragmentShaderSource =
OpenGLVersionStringLiteral +
OpenGLFrameUniformBlockStringLiteral +
OpenGLStructureUniformBlockStringLiteral +
OpenGLLightUniformBlockStringLiteral +
