...
SKCHGCARParser::SKCHGCARParser(QUrl url, bool onlyAsymmetricUnitCell, bool asMolecule, CharacterSet charactersToBeSkipped): SKParser(),
...
void SKCHGCARParser::startParsing()
{
  double3x3 unitCell{};
  ...
  std::shared_ptr<SKCell> cell = std::make_shared<SKCell>(unitCell);
  ...
  if(!succes) {throw std::runtime_error("Could not parse x-dimension");}

  dimensions.y = termsScannedLined[1].toInt(&succes);
  if(!succes) {throw std::runtime_error("Could not parse y-dimension");}

  dimensions.z = termsScannedLined[2].toInt(&succes);
  if(!succes) {throw std::runtime_error("Could not parse z-dimension");}

  double inverseVolume = 1.0/cell->volume();
  ...
  for(int z=0;z<dimensions.z;z++) // Z is the outer loop
  {
    for(int y=0;y<dimensions.y;y++) // Y is the middle loop
    {
      for(int x=0;x<dimensions.x;x++) // X is the inner loop
      {
        ...
        // data in CHGCAR is multiplied by the volume
        double dataPoint = value * inverseVolume;

        if(value>maximumValue) maximumValue = dataPoint;
        if(value<minimumValue) minimumValue = dataPoint;
        sum += dataPoint;
