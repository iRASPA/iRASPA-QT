...
RingMatrix::RingMatrix(int rows, int columns, int initialValue): _rows(rows), _columns(columns), _grid(rows * columns)
{
  std::fill(_grid.begin(), _grid.end(), initialValue);
}

RingMatrix RingMatrix::createRandomRingMatrix(int rows, int columns, int limit)
{
...
RingMatrix::RingMatrix(int rows, int columns, std::vector<std::vector<int>> data): _rows(rows), _columns(columns), _grid(rows * columns)
{
...
void RingMatrix::swapColumns(int a, int b)
{
  for(int i=0;i<this->_rows;i++)
  {
    int temp = (*this)(i,a);
...
RingMatrix RingMatrix::submatrix(int startRow, int startColumn, int numberOfRows, int numberOfColumns)
{
...
void RingMatrix::assignSubmatrix(int startRow, int startColumn, RingMatrix replacement)
{
  for(int i=0;i<replacement._rows;i++)
  {
    for(int j=0;j<replacement._columns;j++)
    {
...
RingMatrix RingMatrix::zeros(int rows, int columns)
{
  return RingMatrix(rows, columns, 0);
}

RingMatrix RingMatrix::ones(int rows, int columns)
{
...
RingMatrix RingMatrix::identity(int size)
{
...
std::tuple<RingMatrix,RingMatrix,std::vector<int>>  RingMatrix::HermiteNormalForm()
{
  ...
  for(int j=1;j<Apad._columns;j++)
  {
    // Search for next rank increase
    bool found = false;
    for(int k=(r + 1);k<Apad._rows;k++)
    {
    ...
    if(found)
    {
      ...
      // Update CC
      for(int i=(r + 1);i<std::get<1>(columnReduction)._columns;i++)
      {
      ...
      for(int i=(r + 1);i<std::get<1>(columnReduction)._columns;i++)
      {
        ...
        {
          for(int j=0;j<QQ._rows;j++)
          {
            QQ(r, j) += std::get<1>(columnReduction)(r, i) * QQ(i, j);
          }
        }
      }
      // Update QQ to Q * QQ
  ...
  rp = std::vector<int>(rp.begin() + 1, rp.begin() + r);  //rp = Array(rp[1..<r])
...
std::tuple<RingMatrix,RingMatrix,RingMatrix> RingMatrix::ColumnReduction(RingMatrix A1, int col_1, int col_2, int row_start)
{
  RingMatrix A = A1;
  int n = A._rows;
  int m = A._columns;
...
std::pair<std::vector<int>,std::vector<int>> RingMatrix::Conditioning(RingMatrix A, int col_1, int col_2, int row_start)
{
  int k = A._rows - row_start - 2;
  ...
  if(d11 * d22 == d12 * d21)
  {
    for(int s=(row_start + 2);s<A._rows;s++)
    {
...
std::variant<std::vector<int>, int> RingMatrix::Algorithm_6_14(int a, int b, int N, std::vector<int> Nfact)
{
  ...
  while(true)
  {
    ...
    else
    {
      ...
      k = int(double(HNF_C_IwaniecLocal) * temp * (pow(log(temp),2)));
    }
    ...
    for(size_t c=0;c<(k+1);c++)
    {
      if(B[c] == true)
      {
        for(size_t i=0;i<t;i++)
        {
          int gi = Ring::greatestCommonDivisor(ai[i] + c * bi[i], Nfact[i]);
...
std::tuple<RingMatrix, RingMatrix, RingMatrix> RingMatrix::SmithNormalForm()
{
  ...
  std::tuple<RingMatrix, RingMatrix, std::vector<int>> hnf = HermiteNormalForm();
...
void RingMatrix::Smith_Theorem5(RingMatrix &A, RingMatrix &U, RingMatrix &V, int col)
{
  int n = A._rows;
  int m = A._columns;
...
void RingMatrix::Smith_Theorem8(RingMatrix &A, RingMatrix &U, RingMatrix &V, int row, int r)
{
  int n = A._rows;
  int m = A._columns;
