...
EllipsoidPrimitive::EllipsoidPrimitive(const EllipsoidPrimitive &ellipsoidPrimitive): Primitive(ellipsoidPrimitive)
{
}
...
std::shared_ptr<Primitive> EllipsoidPrimitive::flattenHierarchy() const
{
...
std::shared_ptr<Primitive> EllipsoidPrimitive::appliedCellContentShift() const
{
...
QDataStream &operator>>(QDataStream &stream, std::shared_ptr<EllipsoidPrimitive> &primitive)
{
  ...
  else
  {
    qint64 magicNumber;
    stream >> magicNumber;
    if(magicNumber != qint64(0x6f6b6192))
    {
      throw InvalidArchiveVersionException(__FILE__, __LINE__, "EllipsoidPrimitive invalid magic-number");
    }
