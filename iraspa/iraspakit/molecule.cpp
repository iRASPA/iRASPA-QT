    ...
    if(isVisible)
    {
      ...
      for(std::shared_ptr<SKBond> bond : bonds)
      {
        if (bond->boundaryType() == SKBond::BoundaryType::internal)
        {
          ...
          double3 pos1 = double3::flip(bond->atom1()->position(), contentFlip, double3(0.0,0.0,0.0)) + contentShift + bond->atom1()->parent()->displacement();
          double3 pos2 = double3::flip(bond->atom2()->position(), contentFlip, double3(0.0,0.0,0.0)) + contentShift + bond->atom2()->parent()->displacement();
...
{
  std::shared_ptr<Molecule> molecule = std::make_shared<Molecule>(static_cast<const Molecule&>(*this));
  ...
  molecule->atomsTreeController()->setTags();
  molecule->reComputeBoundingBox();
  molecule->computeBonds();

  return molecule;
}

std::shared_ptr<Structure> Molecule::appliedCellContentShift() const
{
  ...
  streamRead >> molecule->_atomsTreeController;
  ...
  molecule->setCell(std::make_shared<SKCell>(*_cell));
  molecule->cell()->setContentFlip(bool3(false,false,false));
  molecule->cell()->setContentShift(double3(0.0,0.0,0.0));
  molecule->atomsTreeController()->setTags();
  molecule->reComputeBoundingBox();
  molecule->computeBonds();
...
std::vector<RKInPerInstanceAttributesText> Molecule::atomTextData(RKFontAtlas *fontAtlas) const
{
...
double3x3 Molecule::matrixOfInertia(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms) const
{
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> Molecule::translatedPositionsSelectionCartesian(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, double3 translation) const
{
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> Molecule::translatedPositionsSelectionBodyFrame(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, double3 translation) const
{
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> Molecule::rotatedPositionsSelectionCartesian(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, simd_quatd rotation) const
{
  double3 com = centerOfMassOfSelectionAsymmetricAtoms(atoms);
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> Molecule::rotatedPositionsSelectionBodyFrame(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, simd_quatd rotation) const
{
  double3 com = centerOfMassOfSelectionAsymmetricAtoms(atoms);
