...
BondSelectionIndexSet MolecularCrystal::filterCartesianBondPositions(std::function<bool(double3)> &closure)
{
...
void MolecularCrystal::reComputeBoundingBox()
{
...
std::vector<std::tuple<double3, int, double>> MolecularCrystal::atomSymmetryData() const
{
...
void MolecularCrystal::setAtomSymmetryData(double3x3 unitCell, std::vector<std::tuple<double3, int, double>> atomData)
{
...
std::shared_ptr<Structure> MolecularCrystal::appliedCellContentShift() const
{
  ...
  streamRead >> molecularCrystal->_atomsTreeController;

  const std::vector<std::shared_ptr<SKAtomTreeNode>> asymmetricAtomNodes = molecularCrystal->atomsTreeController()->flattenedLeafNodes();
  ...
  molecularCrystal->setCell(std::make_shared<SKCell>(*_cell));
  molecularCrystal->cell()->setContentFlip(bool3(false,false,false));
  molecularCrystal->cell()->setContentShift(double3(0.0,0.0,0.0));
  molecularCrystal->atomsTreeController()->setTags();
  molecularCrystal->reComputeBoundingBox();
  molecularCrystal->computeBonds();
...
std::shared_ptr<Structure> MolecularCrystal::removeSymmetry() const
{
  // use the copy constructor
  std::shared_ptr<MolecularCrystal> crystal = std::make_shared<MolecularCrystal>(static_cast<const MolecularCrystal&>(*this));
...
std::shared_ptr<Structure> MolecularCrystal::wrapAtomsToCell() const
{
  // use the copy constructor
  std::shared_ptr<MolecularCrystal> crystal = std::make_shared<MolecularCrystal>(static_cast<const MolecularCrystal&>(*this));
...
std::vector<RKInPerInstanceAttributesText> MolecularCrystal::atomTextData(RKFontAtlas *fontAtlas) const
{
  int minimumReplicaX = _cell->minimumReplicaX();
  int minimumReplicaY = _cell->minimumReplicaY();
  int minimumReplicaZ = _cell->minimumReplicaZ();
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> MolecularCrystal::translatedPositionsSelectionCartesian(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, double3 translation) const
{
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> MolecularCrystal::translatedPositionsSelectionBodyFrame(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, double3 translation) const
{
...
double3 MolecularCrystal::centerOfMassOfSelectionAsymmetricAtoms(std::vector<std::shared_ptr<SKAsymmetricAtom>> asymmetricAtoms) const
{
...
double3x3 MolecularCrystal::matrixOfInertia(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms) const
{
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> MolecularCrystal::rotatedPositionsSelectionCartesian(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, simd_quatd rotation) const
{
  double3 com = centerOfMassOfSelectionAsymmetricAtoms(atoms);
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> MolecularCrystal::rotatedPositionsSelectionBodyFrame(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, simd_quatd rotation) const
{
  double3 com = centerOfMassOfSelectionAsymmetricAtoms(atoms);
...
std::vector<float> MolecularCrystal::gridData()
{
...
std::vector<float4> MolecularCrystal::gridValueAndGradientData()
{
...
void MolecularCrystal::setEncompassingPowerOfTwoCubicGridSize(int value)
{
...
double MolecularCrystal::computeVoidFractionAccelerated() const noexcept(false)
{
...
double MolecularCrystal::computeNitrogenSurfaceAreaAccelerated() const noexcept(false)
{
  int3 size = int3(128,128,128);
...
double MolecularCrystal::computeVoidFraction() const noexcept
{
  int3 size = int3(128,128,128);
  double2 probeParameter = double2(10.9, 2.64);
  std::vector<double3> positions = atomUnitCellPositions();
...
double MolecularCrystal::computeNitrogenSurfaceArea() const noexcept
{
  int3 size = int3(128,128,128);
...
void MolecularCrystal::setStructureVolumetricNitrogenSurfaceArea(double value)
{
  _structureVolumetricNitrogenSurfaceArea = value * 1e4 / cell()->volume();
}

void MolecularCrystal::setStructureGravimetricNitrogenSurfaceArea(double value)
{
...
QDataStream &operator>>(QDataStream &stream, std::shared_ptr<MolecularCrystal> &molecularCrystal)
{
  ...
  if(versionNumber >= 2) // introduced in version 2
  {
    ...
    qint64 magicNumber;
    stream >> magicNumber;
    if(magicNumber != qint64(0x6f6b6184))
    {
      throw InvalidArchiveVersionException(__FILE__, __LINE__, "MolecularCrystal invalid magic-number");
    }
  }
