...
{
  _atomsTreeController->setRootNodes(ProteinAtomTreeBuilder::build(frame->atoms, ribbonSecondaryStructureMethod()));
  expandSymmetry();
...
QDataStream &operator>>(QDataStream &stream, std::shared_ptr<Protein> &protein)
{
  ...
  if(versionNumber >= 2) // introduced in version 2
  {
    ...
    {
      throw InvalidArchiveVersionException(__FILE__, __LINE__, "Protein invalid magic-number");
    }
  }
