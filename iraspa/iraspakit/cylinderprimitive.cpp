...
std::vector<RKInPerInstanceAttributesAtoms> CylinderPrimitive::renderSelectedPrimitiveCylinderObjects() const
{
  std::vector<RKInPerInstanceAttributesAtoms> atomData;
  double3 contentShift = _cell->contentShift();
  bool3 contentFlip = _cell->contentFlip();
...
std::set<int> CylinderPrimitive::filterCartesianAtomPositions(std::function<bool(double3)> & closure)
{
  std::set<int> data;
...
std::shared_ptr<Primitive> CylinderPrimitive::flattenHierarchy() const
{
  std::shared_ptr<CylinderPrimitive> cylinderPrimitive = std::make_shared<CylinderPrimitive>(static_cast<const CylinderPrimitive&>(*this));
  ...
  cylinderPrimitive->atomsTreeController()->setTags();
  cylinderPrimitive->reComputeBoundingBox();
  //cylinderPrimitive->computeBonds();

  return cylinderPrimitive;
}

std::shared_ptr<Primitive> CylinderPrimitive::appliedCellContentShift() const
{
  // use the copy constructor
  std::shared_ptr<CylinderPrimitive> cylinderPrimitive = std::make_shared<CylinderPrimitive>(static_cast<const CylinderPrimitive&>(*this));
  cylinderPrimitive->_spaceGroup = this->_spaceGroup;
  ...
  streamRead >> cylinderPrimitive->_atomsTreeController;

  const std::vector<std::shared_ptr<SKAtomTreeNode>> asymmetricAtomNodes = cylinderPrimitive->atomsTreeController()->flattenedLeafNodes();
  ...
  cylinderPrimitive->setCell(std::make_shared<SKCell>(*_cell));
  cylinderPrimitive->cell()->setContentFlip(bool3(false,false,false));
  cylinderPrimitive->cell()->setContentShift(double3(0.0,0.0,0.0));
  cylinderPrimitive->atomsTreeController()->setTags();
  cylinderPrimitive->reComputeBoundingBox();
 // cylinderPrimitive->computeBonds();
...
double3 CylinderPrimitive::centerOfMassOfSelectionAsymmetricAtoms(std::vector<std::shared_ptr<SKAsymmetricAtom>> asymmetricAtoms) const
{
...
double3x3 CylinderPrimitive::matrixOfInertia(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms) const
{
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> CylinderPrimitive::translatedPositionsSelectionCartesian(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, double3 translation) const
{
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> CylinderPrimitive::translatedPositionsSelectionBodyFrame(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, double3 translation) const
{
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> CylinderPrimitive::rotatedPositionsSelectionCartesian(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, simd_quatd rotation) const
{
  double3 com = centerOfMassOfSelectionAsymmetricAtoms(atoms);
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> CylinderPrimitive::rotatedPositionsSelectionBodyFrame(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, simd_quatd rotation) const
{
  double3 com = centerOfMassOfSelectionAsymmetricAtoms(atoms);
