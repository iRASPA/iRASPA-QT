...
DNA::DNA(const DNA &protein): Structure(protein), DNARibbonMixin(protein)
{
}
...
DNA::DNA(const std::shared_ptr<Object> object): Structure(object)
{
  ...
  expandSymmetry();
  _atomsTreeController->setTags();
  DNA::reComputeBoundingBox();
  computeBonds();
  rebuildBackbone();
}

std::shared_ptr<Object> DNA::shallowClone()
{
  return std::make_shared<DNA>(static_cast<const DNA&>(*this));
}

// MARK: Rendering
// =====================================================================

std::vector<RKInPerInstanceAttributesAtoms> DNA::renderAtoms() const
{
...
std::vector<RKInPerInstanceAttributesBonds> DNA::renderInternalBonds() const
{
...
std::vector<RKInPerInstanceAttributesAtoms> DNA::renderSelectedAtoms() const
{
...
std::vector<RKInPerInstanceAttributesBonds> DNA::renderSelectedInternalBonds() const
{
  std::vector<RKInPerInstanceAttributesBonds> data =  std::vector<RKInPerInstanceAttributesBonds>();
  double3 contentShift = _cell->contentShift();
  bool3 contentFlip = _cell->contentFlip();
...
std::set<int> DNA::filterCartesianAtomPositions(std::function<bool(double3)> & closure)
{
  std::set<int> data;
...
BondSelectionIndexSet DNA::filterCartesianBondPositions(std::function<bool(double3)> &closure)
{
...
SKBoundingBox DNA::boundingBox() const
{
...
void DNA::expandSymmetry(std::shared_ptr<SKAsymmetricAtom> asymmetricAtom)
{
  std::vector<std::shared_ptr<SKAtomCopy>> atomCopies = std::vector<std::shared_ptr<SKAtomCopy>>{};
...
std::shared_ptr<Structure> DNA::flattenHierarchy() const
{
  std::shared_ptr<DNA> protein = std::make_shared<DNA>(static_cast<const DNA&>(*this));
...
std::shared_ptr<Structure> DNA::appliedCellContentShift() const
{
...
std::optional<std::pair<std::shared_ptr<SKCell>, double3>> DNA::cellForFractionalPositions()
{
...
std::optional<std::pair<std::shared_ptr<SKCell>, double3>> DNA::cellForCartesianPositions()
{
  return std::nullopt;
}

std::vector<std::shared_ptr<SKAsymmetricAtom>> DNA::asymmetricAtomsCopiedAndTransformedToFractionalPositions()
{
...
std::vector<std::shared_ptr<SKAsymmetricAtom>> DNA::asymmetricAtomsCopiedAndTransformedToCartesianPositions()
{
  std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms{};
...
std::vector<std::shared_ptr<SKAsymmetricAtom>> DNA::atomsCopiedAndTransformedToFractionalPositions()
{
  std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms{};
...
std::vector<std::shared_ptr<SKAsymmetricAtom>> DNA::atomsCopiedAndTransformedToCartesianPositions()
{
  std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms{};
...
double DNA::bondLength(std::shared_ptr<SKBond> bond) const
{
...
double3 DNA::bondVector(std::shared_ptr<SKBond> bond) const
{
  double3 pos1 = bond->atom1()->position();
  double3 pos2 = bond->atom2()->position();
...
std::pair<double3, double3> DNA::computeChangedBondLength(std::shared_ptr<SKBond> bond, double bondLength) const
{
  double3 pos1 = bond->atom1()->position();
...
std::vector<double3> DNA::atomPositions() const
{
  std::vector<std::shared_ptr<SKAtomTreeNode>> asymmetricAtomNodes = _atomsTreeController->flattenedLeafNodes();
...
std::vector<RKInPerInstanceAttributesText> DNA::atomTextData(RKFontAtlas *fontAtlas) const
{
...
double3 DNA::centerOfMassOfSelectionAsymmetricAtoms(std::vector<std::shared_ptr<SKAsymmetricAtom>> asymmetricAtoms) const
{
...
double3x3 DNA::matrixOfInertia(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms) const
{
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> DNA::translatedPositionsSelectionCartesian(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, double3 translation) const
{
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> DNA::translatedPositionsSelectionBodyFrame(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, double3 translation) const
{
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> DNA::rotatedPositionsSelectionCartesian(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, simd_quatd rotation) const
{
  double3 com = centerOfMassOfSelectionAsymmetricAtoms(atoms);
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> DNA::rotatedPositionsSelectionBodyFrame(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, simd_quatd rotation) const
{
  double3 com = centerOfMassOfSelectionAsymmetricAtoms(atoms);
...
QDataStream &operator<<(QDataStream &stream, const std::shared_ptr<DNA> &protein)
{
  stream << qint64(1);
  stream << qint64(0x444e4187);
...
QDataStream &operator>>(QDataStream &stream, std::shared_ptr<DNA> &protein)
{
  qint64 versionNumber;
  stream >> versionNumber;
  if(versionNumber > protein->_versionNumber)
  {
    throw InvalidArchiveVersionException(__FILE__, __LINE__, "DNA");
  }
  ...
  if (versionNumber <= 1)
  {
    qint64 magicNumber;
    stream >> magicNumber;
    if(magicNumber != qint64(0x444e4187))
    {
      throw InvalidArchiveVersionException(__FILE__, __LINE__, "DNA invalid magic-number");
    }
    protein->readCocoaRibbonAppearance(stream);

    std::shared_ptr<Structure> structure = std::static_pointer_cast<Structure>(protein);
    stream >> structure;
    DNAAtomTreeBuilder::applyHierarchyIfNeeded(*protein->atomsTreeController());
  ...
  if(versionNumber >= 2)
  {
    qint64 magicNumber;
    stream >> magicNumber;
    if(magicNumber != qint64(0x6f6b6187))
    {
      throw InvalidArchiveVersionException(__FILE__, __LINE__, "DNA invalid magic-number");
    }
  }
  ...
  protein->readRibbonState(stream, versionNumber);
  if (versionNumber >= 3)
  {
    protein->rebuildBackbone();
  }
