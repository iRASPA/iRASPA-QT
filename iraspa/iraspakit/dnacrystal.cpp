...
DNACrystal::DNACrystal(const DNACrystal &proteinCrystal): Structure(proteinCrystal), DNARibbonMixin(proteinCrystal)
{
  _spaceGroup = proteinCrystal._spaceGroup;
}

DNACrystal::DNACrystal(std::shared_ptr<SKStructure> frame): Structure(frame)
{
...
DNACrystal::DNACrystal(const std::shared_ptr<Object> object): Structure(object)
{
  ...
  if (std::shared_ptr<AtomViewer> atomViewer = std::dynamic_pointer_cast<AtomViewer>(object))
  {
    ...
    {
      DNACrystal::convertAsymmetricAtomsToCartesian();
    }
  }
...
std::shared_ptr<Object> DNACrystal::shallowClone()
{
  return std::make_shared<DNACrystal>(static_cast<const DNACrystal&>(*this));
}

// MARK: Rendering
// =====================================================================

std::vector<RKInPerInstanceAttributesAtoms> DNACrystal::renderAtoms() const
{
...
std::vector<RKInPerInstanceAttributesBonds> DNACrystal::renderInternalBonds() const
{
  int minimumReplicaX = _cell->minimumReplicaX();
  int minimumReplicaY = _cell->minimumReplicaY();
  int minimumReplicaZ = _cell->minimumReplicaZ();
...
std::vector<RKInPerInstanceAttributesAtoms> DNACrystal::renderUnitCellSpheres() const
{
  int minimumReplicaX = _cell->minimumReplicaX();
  int minimumReplicaY = _cell->minimumReplicaY();
  int minimumReplicaZ = _cell->minimumReplicaZ();
...
std::vector<RKInPerInstanceAttributesBonds> DNACrystal::renderUnitCellCylinders() const
{
  int minimumReplicaX = _cell->minimumReplicaX();
  int minimumReplicaY = _cell->minimumReplicaY();
  int minimumReplicaZ = _cell->minimumReplicaZ();
...
std::vector<RKInPerInstanceAttributesAtoms> DNACrystal::renderSelectedAtoms() const
{
  int minimumReplicaX = _cell->minimumReplicaX();
  int minimumReplicaY = _cell->minimumReplicaY();
  int minimumReplicaZ = _cell->minimumReplicaZ();
...
std::vector<RKInPerInstanceAttributesBonds> DNACrystal::renderSelectedInternalBonds() const
{
  int minimumReplicaX = _cell->minimumReplicaX();
  int minimumReplicaY = _cell->minimumReplicaY();
  int minimumReplicaZ = _cell->minimumReplicaZ();
...
std::set<int> DNACrystal::filterCartesianAtomPositions(std::function<bool(double3)> & closure)
{
  std::set<int> data;
...
BondSelectionIndexSet DNACrystal::filterCartesianBondPositions(std::function<bool(double3)> &closure)
{
...
SKBoundingBox DNACrystal::boundingBox() const
{
...
void DNACrystal::reComputeBoundingBox()
{
...
void DNACrystal::expandSymmetry()
{
  std::vector<std::shared_ptr<SKAtomTreeNode>> asymmetricAtomNodes = _atomsTreeController->flattenedLeafNodes();
...
void DNACrystal::expandSymmetry(std::shared_ptr<SKAsymmetricAtom> asymmetricAtom)
{
  std::vector<std::shared_ptr<SKAtomCopy>> atomCopies = std::vector<std::shared_ptr<SKAtomCopy>>{};
...
std::vector<std::tuple<double3, int, double>> DNACrystal::atomSymmetryData() const
{
...
void DNACrystal::setAtomSymmetryData(double3x3 unitCell, std::vector<std::tuple<double3, int, double>> atomData)
{
...
std::shared_ptr<Structure> DNACrystal::flattenHierarchy() const
{
  std::shared_ptr<DNACrystal> proteinCrystal = std::make_shared<DNACrystal>(static_cast<const DNACrystal&>(*this));
...
std::shared_ptr<Structure> DNACrystal::appliedCellContentShift() const
{
...
std::optional<std::pair<std::shared_ptr<SKCell>, double3>> DNACrystal::cellForFractionalPositions()
{
  return std::make_pair(std::make_shared<SKCell>(*_cell), double3(0.0,0.0,0.0));
}

std::optional<std::pair<std::shared_ptr<SKCell>, double3>> DNACrystal::cellForCartesianPositions()
{
  return std::make_pair(std::make_shared<SKCell>(*_cell), double3(0.0,0.0,0.0));
}

std::vector<std::shared_ptr<SKAsymmetricAtom>> DNACrystal::asymmetricAtomsCopiedAndTransformedToCartesianPositions()
{
...
std::vector<std::shared_ptr<SKAsymmetricAtom>> DNACrystal::asymmetricAtomsCopiedAndTransformedToFractionalPositions()
{
  std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms{};
...
std::vector<std::shared_ptr<SKAsymmetricAtom>> DNACrystal::atomsCopiedAndTransformedToCartesianPositions()
{
  std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms{};
...
std::vector<std::shared_ptr<SKAsymmetricAtom>> DNACrystal::atomsCopiedAndTransformedToFractionalPositions()
{
  std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms{};
...
std::shared_ptr<Structure> DNACrystal::superCell() const
{
  qDebug() << "Crystal::superCell";

  // use the copy constructor
  std::shared_ptr<DNACrystal> crystal = std::make_shared<DNACrystal>(static_cast<const DNACrystal&>(*this));
...
std::shared_ptr<Structure> DNACrystal::removeSymmetry() const
{
  // use the copy constructor
  std::shared_ptr<DNACrystal> crystal = std::make_shared<DNACrystal>(static_cast<const DNACrystal&>(*this));
...
std::shared_ptr<Structure> DNACrystal::wrapAtomsToCell() const
{
  // use the copy constructor
  std::shared_ptr<DNACrystal> proteinCrystal = std::make_shared<DNACrystal>(static_cast<const DNACrystal&>(*this));
...
std::vector<double3> DNACrystal::atomPositions() const
{
  int minimumReplicaX = _cell->minimumReplicaX();
  int minimumReplicaY = _cell->minimumReplicaY();
  int minimumReplicaZ = _cell->minimumReplicaZ();
...
std::vector<double3> DNACrystal::atomUnitCellPositions() const
{
  std::vector<std::shared_ptr<SKAtomTreeNode>> asymmetricAtomNodes = _atomsTreeController->flattenedLeafNodes();
...
std::vector<double2> DNACrystal::potentialParameters() const
{
  std::vector<std::shared_ptr<SKAtomTreeNode>> asymmetricAtomNodes = _atomsTreeController->flattenedLeafNodes();
...
double DNACrystal::bondLength(std::shared_ptr<SKBond> bond) const
{
...
double3 DNACrystal::bondVector(std::shared_ptr<SKBond> bond) const
{
  double3 pos1 = bond->atom1()->position();
  double3 pos2 = bond->atom2()->position();
  double3 ds = pos2 - pos1;
...
std::pair<double3, double3> DNACrystal::computeChangedBondLength(std::shared_ptr<SKBond> bond, double bondLength) const
{
  double3 pos1 = bond->atom1()->position();
...
std::vector<RKInPerInstanceAttributesText> DNACrystal::atomTextData(RKFontAtlas *fontAtlas) const
{
  int minimumReplicaX = _cell->minimumReplicaX();
  int minimumReplicaY = _cell->minimumReplicaY();
  int minimumReplicaZ = _cell->minimumReplicaZ();
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> DNACrystal::translatedPositionsSelectionCartesian(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, double3 translation) const
{
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> DNACrystal::translatedPositionsSelectionBodyFrame(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, double3 translation) const
{
...
double3 DNACrystal::centerOfMassOfSelectionAsymmetricAtoms(std::vector<std::shared_ptr<SKAsymmetricAtom>> asymmetricAtoms) const
{
...
double3x3 DNACrystal::matrixOfInertia(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms) const
{
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> DNACrystal::rotatedPositionsSelectionCartesian(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, simd_quatd rotation) const
{
  double3 com = centerOfMassOfSelectionAsymmetricAtoms(atoms);
...
std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> DNACrystal::rotatedPositionsSelectionBodyFrame(std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms, simd_quatd rotation) const
{
  double3 com = centerOfMassOfSelectionAsymmetricAtoms(atoms);
...
std::vector<float> DNACrystal::gridData()
{
...
std::vector<float4> DNACrystal::gridValueAndGradientData()
{
...
void DNACrystal::setEncompassingPowerOfTwoCubicGridSize(int value)
{
...
double DNACrystal::computeVoidFractionAccelerated() const noexcept(false)
{
...
double DNACrystal::computeNitrogenSurfaceAreaAccelerated() const noexcept(false)
{
  int3 size = int3(128,128,128);
...
double DNACrystal::computeVoidFraction() const noexcept
{
  int3 size = int3(128,128,128);
  double2 probeParameter = double2(10.9, 2.64);
  std::vector<double3> positions = atomUnitCellPositions();
...
double DNACrystal::computeNitrogenSurfaceArea() const noexcept
{
  int3 size = int3(128,128,128);
...
void DNACrystal::setStructureVolumetricNitrogenSurfaceArea(double value)
{
  _structureVolumetricNitrogenSurfaceArea = value * 1e4 / cell()->volume();
}

void DNACrystal::setStructureGravimetricNitrogenSurfaceArea(double value)
{
  _structureGravimetricNitrogenSurfaceArea = value * Constants::AvogadroConstantPerAngstromSquared / _structureMass;
}

QDataStream &operator<<(QDataStream &stream, const std::shared_ptr<DNACrystal> &proteinCrystal)
{
  stream << qint64(1);
  stream << proteinCrystal->_spaceGroup;
  stream << qint64(0x444e4185);
...
QDataStream &operator>>(QDataStream &stream, std::shared_ptr<DNACrystal> &proteinCrystal)
{
  qint64 versionNumber;
  stream >> versionNumber;
  if(versionNumber > proteinCrystal->_versionNumber)
  {
    throw InvalidArchiveVersionException(__FILE__, __LINE__, "DNACrystal");
  }
  ...
  if (versionNumber <= 1)
  {
    ...
    qint64 magicNumber;
    stream >> magicNumber;
    if(magicNumber != qint64(0x444e4185))
    {
      throw InvalidArchiveVersionException(__FILE__, __LINE__, "DNACrystal invalid magic-number");
    }
    proteinCrystal->readCocoaRibbonAppearance(stream);

    std::shared_ptr<Structure> structure = std::static_pointer_cast<Structure>(proteinCrystal);
    stream >> structure;
    DNAAtomTreeBuilder::applyHierarchyIfNeeded(*proteinCrystal->atomsTreeController());
  ...
  proteinCrystal->readRibbonState(stream, versionNumber);
  if (versionNumber >= 3)
  {
    proteinCrystal->rebuildBackbone();
  }
