...
#include "renderviewrotatepositionsbodyframesubcommand.h"
#include <QDebug>
#include <algorithm>
...
void RenderViewRotatePositionsBodyFrameSubCommand::redo()
{
  ...
  {
    std::vector<std::shared_ptr<SKAsymmetricAtom>> atoms = atomViewer->atomsTreeController()->selectedObjects();
    std::vector<std::pair<std::shared_ptr<SKAsymmetricAtom>, double3>> translatedPositions = atomViewer->rotatedPositionsSelectionBodyFrame(atoms, _rotation);
