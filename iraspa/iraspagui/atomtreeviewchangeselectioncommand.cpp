...
void AtomTreeViewChangeSelectionCommand::redo()
{
  ...
  {
    emit atomModel->updateSelection();
  }

  if(std::shared_ptr<BondListViewModel> bondModel = _bondModel.lock(); bondModel->isActive(_iraspaStructure))
  {
    emit bondModel->updateSelection();
  }
